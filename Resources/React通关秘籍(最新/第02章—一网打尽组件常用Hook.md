...
### Immer 的柯里化用法 (Curried Producer)

其实 `produce` 还有一个更高级的用法，特别适合 React 的 `setState`。

如果第一个参数传的是函数（recipe），`produce` 会返回一个新的函数。
...
因为 `produce(recipe)` 返回的是一个函数 `(state) => nextState`，正好符合 `setState` 接收的函数签名。这样写代码会更干净。

综上，**在 react 里，只要涉及到 state 的修改，就必须返回新的对象，不管是 useState 还是 useReducer。**

### 为什么必须返回新对象？（浅比较原理）

你可能会想：React 为什么不帮我对比一下属性变没变呢？

因为 React 内部使用的是 `Object.is` 进行**浅比较（Shallow Compare）**。

它只比较对象的**引用地址**是否相同，而不会去递归比较对象内部的属性（深比较）。

- **性能考量**：如果每次 State 变化 React 都要去递归对比复杂的对象树，性能消耗太大了。
- **设计哲学**：React 选择了一条最高效的路径——**通过引用变化来判断数据变化**。

所以：

1.  如果你直接修改旧对象（`obj.a = 1`），引用地址没变，React 使用 `Object.is` 对比发现是同一个对象，认为数据没变，**中止更新（Bailout）**。
2.  如果你创建新对象（`{ ...obj, a: 1 }`），引用地址变了，React 认为数据变了，**触发更新**。

这也就是为什么 React 社区推崇 **Immutable（不可变）** 数据结构的原因。
...
1.  **Ref 对象在组件的整个生命周期内保持不变**，即 `useRef` 返回的对象在每次渲染时都是同一个引用。
2.  **修改 `ref.current` 不会触发组件重新渲染**。这是它与 `useState` 的本质区别。

所以，useRef 的使用场景主要有两个：

1.  **存储 DOM 引用**：如上面的例子，为了调用 DOM 的 API（focus, scroll 等）。
2.  **存储“不需要触发渲染”的可变数据**：比如定时器 ID、上一次的 props/state 值等。

比如，我们可以封装一个 hooks 来记录上一次的 props：
...
这里 `ref.current` 在 `useEffect` 中更新，而 `useEffect` 是在渲染后执行的。所以本次渲染拿到的 `ref.current` 还是上一次的值。
...
在 React 19 中，`forwardRef` 已经被废弃了（或者说不再是必须的）。`ref` 变成了普通的 prop。
...
如果要配合 `useImperativeHandle`，也是一样的，直接用传入的 `ref` 即可：
...
代码变得更简洁了，不再需要 `React.forwardRef` 包裹。
