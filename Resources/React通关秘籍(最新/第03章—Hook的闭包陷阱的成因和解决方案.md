...
为什么会出现闭包陷阱呢？这和 React 的渲染机制有关。

React 的函数组件，每次渲染都会重新执行整个函数。
...
第一次渲染时，执行 App 函数，创建了 `count` 变量（值为 0），并执行 `useEffect`。

`useEffect` 的回调函数引用了当前的 `count` 变量（0）。

由于 `useEffect` 的依赖数组是 `[]`，所以它只会在第一次渲染时执行。

当定时器回调执行时，它引用的是**第一次渲染时**创建的那个闭包中的 `count` 变量。

即便后面 `setCount` 触发了新的渲染，React 会再次执行 `App` 函数：

- 第二次渲染：执行 App 函数，创建**新的** `count` 变量（值为 1）。
- 第三次渲染：执行 App 函数，创建**新的** `count` 变量（值为 2）。

**每一次渲染都有它自己的 Props, State 和事件处理函数。**

可以理解为：

```javascript
// 第一次渲染
function App_Render1() {
  const count = 0
  // ... useEffect ...
  // 定时器引用了这里的 count = 0
}
...
// 第二次渲染
function App_Render2() {
  const count = 1
  // ... useEffect 没走 ...
}
```

React 的这种机制被称为 **"Snapshot"（快照）**。每次渲染就像是一张快照，捕获了当时的状态。

而定时器里的回调函数，一直引用着“第一张快照”里的 `count`，自然永远是 0。

这也是为什么 React 要求我们在 `useEffect` 中声明所有依赖的原因——为了确保 effect 函数里引用的变量是对应最新“快照”的。
