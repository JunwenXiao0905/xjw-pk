...
**注意：在 React 19 中，这个限制依然存在。也就是说，如果你想自己从头实现一个 ErrorBoundary，依然必须使用 Class 组件。**

但是，React 19 提供了 `useActionState` 和 `useOptimistic` 等 hook 可能会在表单场景减少一部分 ErrorBoundary 的使用，但对于全局的组件崩溃捕获，Class 组件形式的 ErrorBoundary 依然是唯一解。

不过在实际开发中，我们几乎不需要自己写这个 Class。社区已经有了成熟的解决方案：`react-error-boundary`。
...
`react-error-boundary` 这个库除了提供 `ErrorBoundary` 组件，还提供了一个很有用的 hook：`useErrorBoundary`。

### 强大的重试机制 (resetErrorBoundary)

很多时候，报错了我们不希望就这么“死”在那里，而是希望能**重试**一下。
比如接口挂了，点击“重试”按钮重新请求。

`react-error-boundary` 完美支持了这个场景：
...
- **FallbackComponent**：接收一个组件，这个组件的 props 里会有 `resetErrorBoundary` 方法。
- **onReset**：当用户点击重试（调用 `resetErrorBoundary`）时，会触发这个回调。你可以在这里做一些清理工作（比如把导致错误的 state 重置回初始值）。

### useErrorBoundary：手动触发错误边界

有时候错误不是在渲染期间抛出的（比如 Promise 的 reject，或者事件处理函数里的错误），React 的 ErrorBoundary 默认是捕获不到的。

这时候可以用 `useErrorBoundary` 来手动把错误“扔”给最近的 Boundary：
...
这样，即使是异步错误，也能统一用 ErrorBoundary 的 UI 来处理了。
...
不过别高兴太早，这个 use 的 hook 在 React 19 中已经**正式发布**了！
...
现在 use 这个 hook 已经正式发布了，大家就可以告别自己写个 state 标识 loading 状态这种方式了，直接用 Suspense。
...
这个 hook 在 React 19 已经正式发布，以后代码里会有大量 Suspense 了。
